	    	...
	    	<tr class="text-center">
	    	  ...
	    	  <td>
	    	    ...
	    	    <c:if test="${vo.userDel != 'NO'}">
	    	    	<font color=red>탈퇴</font><br/>(<font color='blue'><b>${pastDate}</b></font>일경과)
	    	    </c:if>
	    	  </td>
	    	  <td>
...
<ul class="pagination justify-content-center">
  <c:if test="${pag > 1}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/AdminMemberList.ad?pag=1&level=${level}">첫페이지</a></li></c:if>
  <c:if test="${curBlock > 0}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/AdminMemberList.ad?pag=${(curBlock-1)*blockSize + 1}&level=${level}">이전블록</a></li></c:if>
  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize) + blockSize}" varStatus="st">
    <c:if test="${i <= totPage && i == pag}"><li class="page-item active"><a class="page-link bg-secondary border-secondary" href="${ctp}/AdminMemberList.ad?pag=${i}&level=${level}">${i}</a></li></c:if>
    <c:if test="${i <= totPage && i != pag}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/AdminMemberList.ad?pag=${i}&level=${level}">${i}</a></li></c:if>
  </c:forEach>
  <c:if test="${curBlock < lastBlock}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/AdminMemberList.ad?pag=${(curBlock+1)*blockSize+1}&level=${level}">다음블록</a></li></c:if>
  <c:if test="${pag < totPage}"><li class="page-item"><a class="page-link text-secondary" href="${ctp}/AdminMemberList.ad?pag=${totPage}&level=${level}">마지막페이지</a></li></c:if>
</ul>
<!-- 블록페이지 끝 -->
