...
--  도서테이블에 있는 모든 출판사를 검색하시오(단, 같은 출판사의 정보는 1회만 출력하시오)
...
select custId, orderdate as 구매날짜, count(*), sum(saleprice) from orders group by custId order by orderdate;  -- 안되고 있었음.
-- select custId, orderdate as 구매날짜, count(*), sum(saleprice) from orders group by custId, orderdate order by orderdate;
-- select custId, orderdate as 구매날짜 from orders group by custId order by orderdate;  -- 안되고 있었음.
-- select custId from orders group by custId order by orderdate;  -- 안되고 있었음.

-- SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));  -- 일회성(태경님 제공)
-- select c.name 고객, count(*) 수량, sum(o.saleprice) 판매액 from customer c join orders o on c.custid = o.custid group by c.custid; -- 준용님 제공
...
select c.name, b.bookname, o.saleprice, o.orderdate 
  from orders o, books b, customer c 
...
select o.*,c.name,b.bookname from orders o, customer c, book b where o.custid=c.custid and o.bookid=b.bookid order by orderdate desc;
select o.*,c.name,b.bookname from orders o, customer c, book b where o.custid=c.custid and o.bookid=b.bookid order by orderdate desc;
