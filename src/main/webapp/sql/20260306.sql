...
-- 앞에서 분리한 '성'과 '이름'중간에 공백을 두 칸 넣어서 다시 결합후 '성명'이란 필드명으로 출력하시오.(concat())
...
select buseo, sum(pay) as 전체급여 , avg(pay) as 합계 , max(pay) as 최대 , min(pay) as 최소 , count(pay) as 건수 from jikwon group by buseo;
...
select * from buseos b join jikwons j using(buseoId);
...
-- select s.name, k.kwamokName from students2 s Full Outer join kwamok2 k; (X)오라클에서,,
...
-- 먼저 전체 급여평균?
select format(avg(pay)) from sawon;
...
select s.name, s.pay, j.jikmuId from sawon s, jikmu j where pay > (select avg(pay) from sawon);
...
select s.name, b.buseoName from buseos b, sawon s where b.buseoId = s.buseoId;
...
select s.name, b.buseoName from sawon s, (select buseoId, buseoName from buseos where jiyuk='서울') as b where s.buseoId=b.buseoId;   -- 인라인쿼리 사용
select s.name, b.buseoName from sawon s, buseos b where (s.buseoId=b.buseoId) and b.jiyuk='서울';   -- join 사용

-- 사원테이블에서 같은 부서의 월급이 해당부서의 최고월급보다 작은 사람들을 조회해보자.
...
-- 합집합 : UNION ALL -- 중복허용
select * from students3 union all select * from students4;
...
select *, last_value(mechul) over (partition by jikwon order by nalja ROWS BETWEEN unbounded preceding AND unbounded following) as 다음매출 from panme2;
